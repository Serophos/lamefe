...
BOOL CCDDB::GetTrackPositions(CArray<CCDDBTrackPosition, CCDDBTrackPosition&>& tracks, LPCTSTR pszDrive)
{
  ...
  {                      
	...
	TRACE("%d:%d:%d -- %d\n", trackPosition.m_nMinute, trackPosition.m_nSecond, trackPosition.m_nFrame, mciStatusParms.dwReturn);
...
{
  // Get the user name 
  CString sUser("Unknown");
  TCHAR sComputerName[_MAX_PATH];
  DWORD dwSize = _MAX_PATH;
  if (!(::GetUserName(sComputerName, &dwSize)))
   return sUser;
   
  sUser = sComputerName;
  
  // convert string to alpha chars and digits only
  for (int i=0; i<sUser.GetLength(); i++) 
  {
...
CString CCDDB::GetHostName()
{
  ...
  if (gethostname(pszHost, _MAX_PATH) == 0)
   ...
   // convert string to alpha chars and digits only
  for (int i=0; i<sHost.GetLength(); i++) 
  {
...
BOOL CCDDB::ParseRecordBody(LPSTR pszBody, CCDDBRecord& record)
{
  ...
  if (nResponseCode >= 210 && nResponseCode <= 219)
  {
    ...
    do
    {
      ...
      CString sValue;
      if (ParseCommentLine(pszCurLine, sValue))
      {
      ...
      else
      {
        ...
        CString sKeyword;
        if (ParseKeywordLine(pszCurLine, sKeyword, sValue))
        {
