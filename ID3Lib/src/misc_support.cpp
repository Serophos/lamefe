    ...
    if (NULL != tag && NULL != text && strlen(text) > 0)
    {
        ...
        if (bReplace || tag->Find(ID3FID_CDID) == NULL)
        {
            ...
            if (pFrame)
            {
                ...
                mbstoucs( temp, text, nStrLen + 1 );

                pFrame->Field(ID3FN_DATA).Set(	(const uchar *)temp, 
												(nStrLen + 1) * sizeof( unicode_t ) );
...
ID3_Frame* ID3_MyAddTrack(ID3_Tag *tag, uchar ucTrack, uchar ucTotal, bool bReplace, int myType)
{
  ID3_Frame* pFrame = NULL;
  if (NULL != tag && ucTrack > 0)
  {
    ...
    if (bReplace || NULL == tag->Find(ID3FID_TRACKNUM))
    {
      ...
      if (trackFrame)
      {
		...
		switch ( myType )
		{
			case 0: sprintf( sTrack, "%lu", (luint) ucTrack ); break;
			case 1: sprintf( sTrack, "%lu/%lu", (luint) ucTrack, (luint) ucTotal ); break;
			case 2: sprintf( sTrack, "%02lu", (luint) ucTrack ); break;
			case 3: sprintf( sTrack, "%02lu/%02lu", (luint) ucTrack, (luint) ucTotal ); break;
			default:
				assert( false );
        }
...
ID3_Frame* ID3_AddGenre(ID3_Tag *tag, char* lpszGenre, bool bReplace)
{
  ID3_Frame* pFrame = NULL;
  if ( NULL != lpszGenre )
  {
    ...
    if (bReplace || NULL == tag->Find(ID3FID_CONTENTTYPE))
    {
      ...
      if (NULL != pFrame)
      {
        pFrame->Field(ID3FN_TEXT) = lpszGenre;
        tag->AttachFrame(pFrame);
      }
    }
  }
