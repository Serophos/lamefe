...
#if defined WIN32
#  include <windows.h>
static int truncate(const tchar *path, size_t length)
{
  int result = -1;
  HANDLE fh;
...
ID3_Err ID3_Tag::CreateFile()
{
  ...
  // Create a new file
  __file_handle = tfopen(__file_name, _T( "wb+" ) );
...
ID3_Err ID3_Tag::OpenFileForWriting()
{
  ...
  if (exists(__file_name))
  {
    ...
    __file_handle = tfopen(__file_name, _T( "rb+" ) );
  }
  else
  {
    return ID3E_NoFile;
...
size_t ID3_Tag::Link(const tchar *fileInfo, bool parseID3v1, bool parseLyrics3)
{
 ...
 **/
size_t ID3_Tag::Link(const tchar *fileInfo, flags_t tag_types)
{
