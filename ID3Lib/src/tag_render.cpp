...
#define PATCH_TEMPFILE
#ifdef PATCH_TEMPFILE
	// section different from the original idlib 3.7
	// to correct a bug preventing the tag to saved
	// by limited users

	const tchar	sTmpSuffix[] = _T( ".X" );
	tchar		sTempFile[ ID3_PATH_LENGTH ] = { '\0', };
	FILE*		pFile = NULL;

	// check if filename of temp file exceeds maximum filename length
	if ( tstrlen( __file_name ) + tstrlen( sTmpSuffix ) >= ID3_PATH_LENGTH ) 
	{
		ID3_THROW_DESC(ID3E_NoFile, "filename too long");
	}

	// create temp file name
	tstrcpy( sTempFile, __file_name );
	tstrcat( sTempFile, sTmpSuffix );

	if ( NULL != ( pFile = tfopen( sTempFile, _T( "wb" ) ) ) )
	{
		uchar buffer2[ BUFSIZ ];

		// write ID3 tag info to temp file
		fwrite( buffer, 1, size, pFile );

		// seek to correct offset position in the original file
		fseek( __file_handle, __starting_bytes, SEEK_SET );

		// copy original file data
		while (! feof(__file_handle))
		{
			// read two bytes from orignal file
			size_t nBytes = fread(buffer2, 1, BUFSIZ, __file_handle);

			// write two bytes to temp file
			fwrite( buffer2, 1, nBytes, pFile );
		}
		...
		// move the temp file to the orginal filename
		trename( sTempFile, __file_name );

		// re-open the original file again
		OpenFileForWriting();
	}
    ...
    rewind(tempOut);
    tfreopen(__file_name, _T( "wb+" ), __file_handle);
